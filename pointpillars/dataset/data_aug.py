...
def dbsample(CLASSES, data_root, data_dict, db_sampler, sample_groups):
    ...
    for name, v in sample_groups.items():
        ...
        #print(f"v= {v}")
        #print(f"sum = {np.sum(gt_names == name)}")
        sampled_num = v - np.sum(gt_names == name)
        #print(f"Available samples for class {name}: {db_sampler[name].available_samples()}")
        #if db_sampler[name].available_samples() < sampled_num:
            #print(f"Warning: Not enough samples available for class {name}. Needed {sampled_num}, but got {db_sampler[name].available_samples()}")
        #print(f"Class {name}, requested samples: {v}, current gt count: {np.sum(gt_names == name)}, sampled_num: {sampled_num}")
        #print(f"sampled num = {sampled_num}")
        if sampled_num <= 0:
            continue

        # 2. sample databases bboxes
        #print(f"sampled_num = {sampled_num}")
        #print(f"Available classes in db_sampler: {db_sampler.keys()}")
        #print(f"Number of available samples for {name}: {len(db_sampler[name].available_samples())}")
        sampled_cls_list = db_sampler[name].sample(sampled_num)
        #print(f"Sampled bounding boxes: {len(sampled_cls_list)}")
        #print(sampled_cls_list)
        sampled_cls_bboxes = np.array([item['box3d_lidar'] for item in sampled_cls_list], dtype=np.float32)
        #print(sampled_cls_bboxes)
        # 3. box_collision_test
        #print(sampled_cls_bboxes.shape)
        ...
        #print(sampled_cls_bboxes_bv_corners.shape)
        ...
        for i in range(n_gt, len(coll_mat)):
            if any(coll_mat[i]):
                ...
                #print(f"sampled points cur{sampled_pts_cur}")
                sampled_pts_cur[:, :3] += cur_sample['box3d_lidar'][:3]
                sampled_pts.append(sampled_pts_cur)
                #print(f"sampled points {sampled_pts}")
        ...
        avoid_coll_boxes = np.concatenate([avoid_coll_boxes, tmp_bboxes], axis=0)
            ...
            #print(f"Warning: No sampled points for class {name}.")
        #else:
            #print(f"Found {len(sampled_pts)} sampled points for class {name}.")
        
    # merge sampled database
    # remove raw points in sampled_bboxes firstly
    #print(sampled_bboxes)
    #print(gt_names)
    
    #if len(sampled_bboxes) < 1:
        #print("sample_bboxes is empty")
    #else: 
        #print("not empty")
    ...
    #print(sampled_pts)
    if len(sampled_pts) == 0:
        #print("[Warning] No valid samples found after filtering collisions.")
        return data_dict
...
def data_augment(CLASSES, data_root, data_dict, data_aug_config):
    ...
    #print("GT boxes centers before filter (x, y):")
    #print(data_dict['gt_bboxes_3d'][:, :2])
    #print(f"Object range filter bounds: {object_range}")
