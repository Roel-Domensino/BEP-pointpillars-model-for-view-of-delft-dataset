...
def radar2camera(parts,transforms):
    ...
    y_bottom_centered = center_camera[1]  # Add half height to go from center to bottom
...
def iou3d_camera(bboxes1, bboxes2):
    '''
    bboxes1: (n, 7), (x, y, z, h, w, l, theta)
    bboxes2: (m, 7)
    return: (n, m)
    '''
    # 1. height overlap (height = bboxes[:,3])
    bboxes1_bottom = bboxes1[:, 1] - bboxes1[:, 3]  # y - h (assuming y is vertical)
    bboxes2_bottom = bboxes2[:, 1] - bboxes2[:, 3]
    bboxes1_top = bboxes1[:, 1]
    bboxes2_top = bboxes2[:, 1]
    ...
    # 2. bev overlap
    # width = bboxes[:, 4], length = bboxes[:, 5]
    bboxes1_x1y1 = bboxes1[:, [0, 2]] - bboxes1[:, [4, 5]] / 2
    bboxes1_x2y2 = bboxes1[:, [0, 2]] + bboxes1[:, [4, 5]] / 2
    bboxes2_x1y1 = bboxes2[:, [0, 2]] - bboxes2[:, [4, 5]] / 2
    bboxes2_x2y2 = bboxes2[:, [0, 2]] + bboxes2[:, [4, 5]] / 2
    bboxes1_bev = torch.cat([bboxes1_x1y1, bboxes1_x2y2, bboxes1[:, 6:]], dim=-1)
    bboxes2_bev = torch.cat([bboxes2_x1y1, bboxes2_x2y2, bboxes2[:, 6:]], dim=-1)
    bev_overlap = boxes_overlap_bev(bboxes1_bev, bboxes2_bev)  # (n, m)

    # 3. overlap and volume
    volume1 = bboxes1[:, 3] * bboxes1[:, 4] * bboxes1[:, 5]  # h * w * l
    volume2 = bboxes2[:, 3] * bboxes2[:, 4] * bboxes2[:, 5]
    volume = volume1[:, None] + volume2[None, :]

    # 4. iou
    iou = height_overlap * bev_overlap / (volume - height_overlap * bev_overlap + 1e-8)
...
def keep_bbox_from_image_range(result, tr_velo_to_cam, r0_rect, P2, image_shape,idx):
    '''
    ...
    '''
    h, w = image_shape
    root_dir=root_dir = "/home/student2/Documents/Datasets/View_of_Delft_dataset_PUBLIC/view_of_delft_PUBLIC"
    kitti_locations = KittiLocations(root_dir=root_dir)
    frame_data = FrameDataLoader(kitti_locations=kitti_locations, frame_number=str(idx).zfill(5))
    transforms = FrameTransformMatrix(frame_data)

    lidar_bboxes = result['lidar_bboxes']
    labels = result['labels']
    scores = result['scores']
    camera_bboxes = np.zeros_like(lidar_bboxes) # (n, 7)
    #camera_bboxes = bbox_lidar2camera(lidar_bboxes, tr_velo_to_cam, r0_rect) # (n, 7)
    print(f"lidar_bboxes idx: {idx}, box: {lidar_bboxes}")
    for i in range(len(lidar_bboxes)):
        camera_bboxes[i] = radar2camera(lidar_bboxes[i],transforms)
